...
def generate_book_id():
    first_three_chars = 'LIB'
    remaining_chars_length = 5
...
# Function to generate Member ID as a 5-digit number with leading zeros
...
categories = ['Fiction', 'Non-Fiction', 'Science', 'History',
              'Biography', np.nan]  # Including missing values
category_data = np.random.choice(categories, n_books)

# Ordinal data: book condition
conditions = ['Excellent', 'Good', 'Fair',
              'Poor', np.nan]  # Including missing values
condition_data = np.random.choice(
    conditions, n_books, p=[0.4, 0.3, 0.2, 0.1, 0.0])
...
book_price[np.random.choice(n_books, size=int(0.1*n_books))] = np.nan
...
borrowers = np.random.choice(member_ids,n_borrowing_records)
...
borrowing_records_df['Book_ID'] = np.random.choice(
    book_id_data, n_borrowing_records)

# Simulate return dates
borrowing_records_df['Return_Date'] = borrowing_records_df['Due_Date'] - \
    pd.to_timedelta(np.random.randint(1, 30, n_borrowing_records), unit='D')


# Function to generate Alphanumeric Postcode with 6 characters starting with "AXP"
def generate_postcode():
    return 'AXP' + ''.join(random.choices(string.ascii_uppercase + string.digits, k=4))


member_names = [fake.name() for _ in range(n_members)]
postcodes = [generate_postcode() for _ in range(n_members)]

# Introduce 15% duplicates to postcodes
num_duplicates = int(n_members * 0.15)
duplicate_indices = np.random.choice(
    range(n_members), size=num_duplicates, replace=False)
for index in duplicate_indices:
    postcodes[index] = np.random.choice(postcodes)

fines = np.random.choice([0, 5, 10, np.nan], size=n_members, p=[
                         0.6, 0.1, 0.1, 0.2])  # Including missing values
...
member_details_df.to_csv('members.csv', index=False)
...
member_details_csv_file = 'members.csv'
database_name = 'library_database'

# Call the function for each table with constraints
create_and_import_to_database_with_constraints(
    books_csv_file, database_name, 'books')
create_and_import_to_database_with_constraints(
    borrowing_records_csv_file, database_name, 'borrowing_records')
create_and_import_to_database_with_constraints(
    member_details_csv_file, database_name, 'member_details')
